...
\begin{document}
	...
	\begin{figure}[htbp]
		\centering
		\includegraphics[width=0.7\textwidth]{The-image-is-decomposed-into-four-sub-bands-LL-HL-LH-and-HH-by-1-level-wavelet.png}
	...
	\subsubsection{Geometric Invariance Properties}
	
	The DFT possesses two properties that give DFT-based steganography a natural robustness
	advantage over DCT and DWT methods against certain geometric distortions
	\cite{ruanaidh1998rotation, bracewell2000fourier}:
	
	\begin{itemize}
		\item \textbf{Translation invariance:} A spatial shift of the image by $(x_0, y_0)$
		only introduces a phase factor into each DFT coefficient; the magnitude spectrum
		is completely unchanged:
		\begin{equation}
			\text{DFT}\{I(x-x_0, y-y_0)\} = F(u,v)\cdot e^{-j2\pi(ux_0/M + vy_0/N)}.
			\label{eq:dft_translation}
		\end{equation}
		Because embedding is in the magnitude, spatially translated stego-images can be
		extracted without any re-synchronisation.
		
		\item \textbf{Rotation correspondence:} A spatial rotation of the image by angle $\theta$
		rotates the magnitude spectrum by the same angle $\theta$. If embedding is placed in a
		circularly symmetric ring $\mathcal{R}$, the embedded coefficients remain in the same
		ring after rotation, so a rotation-invariant extractor can still locate them
		\cite{solachidis2001circularly}.
	\end{itemize}
	
	% --- FIGURE 3: Rotation invariance (drawable with TikZ) ---
	\begin{figure}[htbp]
		\centering
		\begin{tikzpicture}[scale=0.95]
			
			% ---- LEFT: original spectrum ----
			\node[font=\small\bfseries] at (2.2, 2.8) {Original spectrum};
			\fill[gray!8] (0,0) rectangle (4.4,4.4);
			\draw[gray!40] (0,0) rectangle (4.4,4.4);
			\fill[blue!8]   (2.2,2.2) circle (1.9);
			\fill[white]    (2.2,2.2) circle (0.55);
			\fill[yellow!10](2.2,2.2) circle (0.55);
			% Embedding ring
			\draw[orange!80!black, very thick, dashed] (2.2,2.2) circle (1.3);
			\draw[orange!80!black, very thick, dashed] (2.2,2.2) circle (0.85);
			% Embedded coefficient
			\fill[red!70] (3.1, 2.7) circle (0.11);
			\node[font=\footnotesize\bfseries, red!70, right] at (3.2, 2.7) {$E$};
			% Ring label with pointer arrow
			\draw[->, gray!60, thin] (3.5, 3.5) -- (2.9, 3.0);
			\node[font=\footnotesize, orange!80!black, align=left] at (3.9, 3.7)
			{embedding\\ring $\mathcal{R}$};
			\node[font=\footnotesize, gray!55] at (2.2, 2.2) {DC};
			\node[font=\footnotesize] at (2.2, -0.2) {$u$};
			\node[font=\footnotesize] at (-0.25, 2.2) {$v$};
			
			% ---- ARROW ----
			\draw[-{Stealth[length=6pt]}, very thick, gray!55] (4.7, 2.2) -- (5.6, 2.2);
			\node[font=\footnotesize, gray!60, above] at (5.15, 2.2) {rotate $\theta$};
			
			% ---- RIGHT: after rotation ----
			\node[font=\small\bfseries] at (8.6, 2.8) {After rotation by $\theta$};
			\fill[gray!8] (6.2,0) rectangle (10.6,4.4);
			\draw[gray!40] (6.2,0) rectangle (10.6,4.4);
			\fill[blue!8]   (8.4,2.2) circle (1.9);
			\fill[white]    (8.4,2.2) circle (0.55);
			\fill[yellow!10](8.4,2.2) circle (0.55);
			% Same ring — unchanged
			\draw[orange!80!black, very thick, dashed] (8.4,2.2) circle (1.3);
			\draw[orange!80!black, very thick, dashed] (8.4,2.2) circle (0.85);
			% Rotated embedded point
			\fill[red!70] (8.8, 3.35) circle (0.11);
			\node[font=\footnotesize\bfseries, red!70, right] at (8.9, 3.35) {$E'$};
			\node[font=\footnotesize, gray!55] at (8.4, 2.2) {DC};
			\node[font=\footnotesize] at (8.4, -0.2) {$u$};
			\node[font=\footnotesize] at (5.9, 2.2) {$v$};
			\node[font=\footnotesize, teal!70!black] at (8.4, -0.5)
			{Ring $\mathcal{R}$ unchanged; $E'$ still inside};
			
		\end{tikzpicture}
		\caption{Rotation invariance of DFT magnitude-based embedding. Spatial rotation by angle
			$\theta$ rotates the entire magnitude spectrum by the same angle, moving the
			embedded coefficient from $E$ to $E'$. Because the embedding ring $\mathcal{R}$
			is circularly symmetric (dashed orange), $E'$ remains inside $\mathcal{R}$
			regardless of $\theta$, allowing extraction without any geometric
			re-synchronisation \cite{solachidis2001circularly}.}
		\label{fig:dft_rotation}
	\end{figure}
