...
a bounded number of corrupted bits without any knowledge of the original image \cite{cox2007}.
...
preferable \cite{petitcolas1999}.
...
at unknown locations \cite{cox2007}.
...
steganography \cite{ruanaidh1998}.
...
\begin{itemize}
  \item \textbf{Cropping} removes entire image regions, permanently destroying any fragments
        ...
        the coordinate grid used during embedding \cite{petitcolas1999}.
  ...
  \item \textbf{Rotation} introduces a global angular offset. Unless the embedding domain
        ...
        extraction fails entirely \cite{ruanaidh1998b}.
...
image, and inverts it before extraction \cite{ruanaidh1998}.
...
reliably re-detected after geometric distortion \cite{ruanaidh1998b}.
...
scaling, a property exploited by several watermarking systems \cite{ruanaidh1998b,
...
embedding capacity compared with block-based transforms \cite{petitcolas1999}.
...
and can therefore tolerate stronger embedding \cite{petitcolas1999}.
...
psychophysical experiments \cite{cox2007}.
...
recovery from whatever fragments remain \cite{petitcolas1999, cox2007}.
...
values of neighboring fragments \cite{cox2007}.
...
modifications \cite{Piva2002, petitcolas1999}.
Similarly, distributing redundant copies across the luminance and chrominance channels
...
steganalysts \cite{provos2003, johnson1998exploring}.
...
often increase detectability \cite{Fridrich2012, petitcolas1999}.
...
Chi-squared analysis \cite{provos2003} detects the pairing of adjacent pixel values
caused by LSB substitution.
RS analysis examines the ratio of regular and singular pixel groups, which is disrupted
predictably by LSB embedding \cite{provos2003}.
...
steganalytic feature spaces \cite{fridrich2012, petitcolas1999}.
...
Recent years have seen the application of deep learning to steganographic embedding and
extraction, with the goal of jointly optimizing all three objectives—imperceptibility,
capacity, and robustness—through end-to-end training on large image datasets
\cite{zhu2018hidden, hayes2017}.

\subsubsection{Encoder–Decoder Architectures}

The dominant paradigm is an encoder–decoder architecture in which a convolutional neural
network (CNN) encoder embeds the payload into the cover image, and a paired CNN decoder
recovers it \cite{zhu2018hidden}.
The HiDDeN framework of Zhu et al.\ \cite{zhu2018hidden} introduces a differentiable noise
layer between the encoder and decoder during training, simulating common attacks
(JPEG compression, Gaussian noise, cropping) so that the network learns embeddings that
survive them.
The adversarial component of HiDDeN adds a discriminator network that penalizes
statistically detectable stego-images, incorporating a steganalysis-like signal directly
into the training objective.

\subsubsection{Generative Adversarial Approaches}

Hayes and Danezis \cite{hayes2017} proposed a generative adversarial framework in which
a steganographer network and a steganalyst network compete, with the steganographer
learning to produce stego-images that are simultaneously imperceptible, robust, and
undetectable.
This adversarial training paradigm has since been extended by multiple groups, with
improvements in robustness against geometric attacks through the use of spatial
transformer networks and learned re-synchronization modules \cite{ye2017}.

\subsubsection{Limitations in the Context of This Work}

Despite promising results under trained conditions, deep learning–based approaches
present several limitations that reduce their suitability for the robustness objectives
of this thesis.

\begin{enumerate}
  \item \textbf{Distribution shift.} Neural steganographic systems are robust only to the
        specific attacks simulated during training. Exposure to unseen attack combinations
        or parameter values outside the training distribution causes unpredictable and
        often catastrophic performance degradation \cite{ye2017}.

  \item \textbf{Opacity.} Unlike classical transform-domain methods, where embedding
        positions, redundancy levels, and reconstruction strategies are explicitly defined
        and analyzable, neural networks operate as black-box models. This opacity makes
        it impossible to reason analytically about synchronization behavior, payload
        survivability under geometric attacks, or the probability of partial recovery
        under a given loss model.

  \item \textbf{Resource requirements.} Training competitive deep steganographic models
        requires large-scale datasets and substantial computational infrastructure,
        limiting applicability in constrained or low-resource environments.

  \item \textbf{Interpretability of failure.} When a deep steganographic system fails
        to recover the payload, there is no principled mechanism to determine how much
        of the payload survived or to reconstruct a partial message. The binary
        succeed/fail nature of neural decoding is incompatible with the
        partial-recovery objective of this thesis.
\end{enumerate}

For these reasons, this thesis adopts a classical, model-driven framework rather than
a data-driven one, while acknowledging deep learning steganography as a promising
...
In real-world communication environments, stego-images are rarely transmitted or stored in their original form. Instead, they are frequently subjected to various distortions caused by image processing operations, platform-specific optimizations, or intentional tampering. These distortions can be modeled as attacks that aim, either deliberately or unintentionally, to disrupt the embedded information. A robust steganographic system must therefore be evaluated under realistic attack models that reflect practical usage scenarios rather than ideal transmission conditions \cite{johnson1998exploring,cox2007digital}.

From a robustness perspective, image attacks can be broadly classified into signal processing attacks and geometric attacks. While both categories threaten the integrity of hidden data, they affect steganographic systems in fundamentally different ways.

\subsection{Signal Processing Attacks}

Signal processing attacks modify the pixel intensity values of an image while preserving its overall geometric structure. These attacks primarily affect the frequency content of the image and pose a serious challenge to steganographic methods that rely on fragile pixel-level or coefficient-level modifications \cite{provos2003hide}.

One of the most common signal processing attacks is lossy compression, particularly JPEG compression. JPEG operates by transforming the image into the DCT domain and applying quantization to the frequency coefficients. During this process, high-frequency and some mid-frequency components are significantly altered or discarded, which can lead to partial or complete loss of embedded data, especially when the hidden information is stored in unstable coefficients \cite{wallace1992jpeg,Fridrich2012}. Even moderate compression levels can introduce substantial bit errors in conventional steganographic schemes.

Additive noise represents another significant class of signal processing attacks. Noise sources such as Gaussian noise or salt-and-pepper noise introduce random perturbations to pixel values, which propagate into the transform domain and distort the embedded signal. These random variations often result in bit-level corruption, reducing extraction accuracy and increasing the bit error rate \cite{cox2007digital}. Noise attacks are particularly harmful to low-strength embedding schemes designed primarily for imperceptibility.

Filtering operations, including low-pass, median, and smoothing filters, further degrade embedded information by attenuating high-frequency components and suppressing fine image details. Since many steganographic techniques embed data in frequency bands associated with edges or textures, filtering can significantly weaken or erase the hidden signal \cite{gonzalez2008digital}. Collectively, signal processing attacks introduce cumulative distortions that can severely compromise data recovery when multiple operations are applied sequentially.

\subsection{Geometric Attacks}

Geometric attacks alter the spatial structure of an image, resulting in changes to pixel positions and spatial relationships. Unlike signal processing attacks, geometric attacks do not necessarily destroy the embedded data itself but instead disrupt the synchronization between the embedding and extraction processes \cite{Piva2002}.

Cropping is one of the most destructive geometric attacks in the context of robust steganography. By removing portions of the image, cropping leads to irreversible loss of embedded fragments located within the removed regions. For block-based or location-dependent embedding schemes, cropping causes partial data loss and prevents correct reconstruction of the hidden message \cite{johnson1998exploring}.

Resizing and scaling operations modify the spatial resolution of the image through interpolation or resampling. These operations change pixel alignment and block boundaries, making it difficult to locate the original embedding positions during extraction. Even when the embedded data remains partially intact, resampling introduces interpolation artifacts that degrade extraction reliability \cite{ruanaidh1998rotation}.

Rotation introduces additional complexity by altering the orientation of spatial features. Rotational transformations cause global misalignment, which is particularly problematic for steganographic methods that rely on fixed coordinate systems or block-based partitioning. Without explicit synchronization mechanisms or invariant representations, rotation typically renders extraction infeasible \cite{cox2007digital}.

\subsection*{Steganalysis and Detectability Considerations}

In addition to robustness against image degradation, steganographic systems are also subject to intentional detection attacks, commonly referred to as steganalysis. Steganalysis aims to determine whether a given image contains hidden information by exploiting statistical inconsistencies, structural artifacts, or deviations from natural image models introduced during the embedding process \cite{provos2003hide,Fridrich2012}. Modern steganalysis techniques employ a wide range of methods, including histogram analysis, feature-based classifiers, and machine learning approaches, which can effectively detect many high-capacity or poorly designed steganographic schemes.

It is important to distinguish between robustness and detectability in the context of steganographic design. Robust steganography focuses on preserving the embedded payload under image degradation and tampering, whereas steganalysis resistance aims to minimize the statistical detectability of hidden communication. These objectives are often conflicting, as strategies that improve robustness—such as redundancy, stronger embedding, or structured payload distribution—may increase the risk of detection by altering statistical properties of the cover image \cite{cox2007digital}.

This thesis does not aim to address steganalysis resistance explicitly. Instead, the primary focus is on enhancing payload survivability under realistic signal processing and geometric distortions. While detectability is an important aspect of steganographic security, it is treated as a secondary concern in this work. The inherent trade-off between robustness and undetectability is acknowledged, and the proposed framework prioritizes reliable data recovery in hostile environments over minimizing statistical traces of embedding.


\subsection{Impact of Attacks on Steganographic Robustness}

From a system-level perspective, signal processing attacks primarily result in bit-level corruption of embedded data, whereas geometric attacks cause desynchronization between the embedding and extraction stages. While error correction coding and redundancy can partially mitigate the effects of signal processing distortions, synchronization loss caused by geometric attacks remains a major unresolved challenge in conventional steganographic systems \cite{Piva2002,Fridrich2012}.

Furthermore, real-world scenarios often involve compound attacks, such as compression followed by cropping or resizing, which exacerbate the weaknesses of traditional embedding strategies. These observations highlight the limitations of approaches that focus solely on protecting embedding locations and motivate the need for robustness-oriented frameworks that emphasize payload survivability and partial recovery under severe and combined degradation conditions.
...
\begin{figure}[htbp]
  ...
  \begin{tikzpicture}[
      ...
      noisy/.style  = {draw, dashed, rounded corners=4pt, minimum width=2.4cm,
                       minimum height=0.9cm, align=center, font=\small, fill=yellow!15},
      arr/.style    = {-{Stealth[length=5pt]}, thick}
    ]
    % Nodes
    \node[box, fill=blue!12]  (cover)   {Cover Image\\$C$};
    \node[box, fill=green!12, right=of cover] (enc) {Encoder\\$E_\theta$};
    \node[box, fill=orange!12, right=of enc]  (stego) {Stego-Image\\$S$};
    \node[noisy, right=of stego] (noise) {Noise / Attack\\Layer};
    \node[box, fill=purple!12, right=of noise] (dec)  {Decoder\\$D_\phi$};
    \node[box, fill=red!12, right=of dec]     (msg)  {Recovered\\Message $\hat{M}$};

    \node[box, fill=green!12, below=1.0cm of enc] (secret) {Secret\\Message $M$};
    \node[box, fill=gray!15,  below=1.0cm of noise] (disc) {Discriminator\\(adversarial)};
    ...
    \draw[arr] (dec)    -- (msg);
    \draw[arr] (stego)  -- (disc);
    \draw[arr] (cover)  to[bend right=20] node[below, font=\footnotesize]{$\mathcal{L}_\text{img}$} (stego);

    % Loss label
    \draw[arr, dashed] (msg) to[bend left=40] node[above, font=\footnotesize]{$\mathcal{L}_\text{msg}$} (secret);
  \end{tikzpicture}
  \caption{Generic encoder--decoder architecture for deep steganography. The noise layer
           simulates signal processing and geometric attacks during training, forcing the encoder
           to learn embeddings that survive realistic degradation. An optional discriminator
           provides an adversarial loss $\mathcal{L}_{\text{adv}}$ to penalise statistically
           detectable stego-images.}
...
\begin{figure}[htbp]
  ...
  \begin{tikzpicture}[
    ...
    \node[obj] (rob)  at (5.5,0)  {Robustness};
    \node[obj] (cap)  at (11,0)   {Capacity};

    % Metrics under Imperceptibility
    \node[met] (mse)  at (-1.2,-2.2) {MSE};
    \node[met] (psnr) at (1.2,-2.2)  {PSNR (dB)};
    \node[met] (ssim) at (0,-3.6)    {SSIM};

    % Metrics under Robustness
    \node[met] (ber)  at (4.3,-2.2)  {BER (\%)};
    \node[met] (nc)   at (6.7,-2.2)  {NC};
    \node[met] (prr)  at (5.5,-3.6)  {PRR (proposed)};

    % Metric under Capacity
    \node[met] (bpp)  at (11,-2.2)   {bpp (bits/pixel)};
    ...
    \draw[->, thick, red!60, bend right=15]
      (imp.south east) to node[below, font=\footnotesize, red!60]{trade-off} (rob.south west);
    \draw[->, thick, red!60, bend left=15]
      (rob.south east) to node[below, font=\footnotesize, red!60]{trade-off} (cap.south west);
  \end{tikzpicture}
...
\chapter{Implementation smdljkf and Experimental Setup}
